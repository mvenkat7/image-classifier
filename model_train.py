...
# Function transform_process(train_dir) performs training transformations on a dataset
def transform_process(train_dir):
...
def validation(model, testloader, criterion, device):
    ...
    for second, (inputs, labels) in enumerate(testloader):
...
def network_trainer(model, Trainloader, testloader, device, 
    ...
    for e in range(epochs):
        ...
        for second, (inputs, labels) in enumerate(Trainloader):
            steps += 1
...
def main():
    ...
    valid_data = transform_process(valid_dir)
    test_data = transform_process(test_dir)
